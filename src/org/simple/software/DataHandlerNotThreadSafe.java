...
public class DataHandlerNotThreadSafe implements DataHandler{
    ...
    public DataHandlerNotThreadSafe(boolean cMode, int clientId) {
        this.cMode = cMode;
    ...
    public boolean receiveData(int clientId, String dataChunk) {
        ...
        if (indexNL==0) {
            HelperFunctions.print(DataHandlerNotThreadSafe.class, "SEP@", indexNL+"", " bufdata:\n", bufData);
        }

        if (rest != null) {
            HelperFunctions.print(DataHandlerNotThreadSafe.class, "more than one line: \n", rest);
            try {
                System.in.read();
            } catch (IOException e) {
                e.printStackTrace();
